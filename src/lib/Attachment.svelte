...
{#if attachment.type == 'img'}
	<button class="attachment-button" on:click={toggleFullView}>
		<img
			src={attachment.url}
			alt={attachment.alt}
			class:unloaded={attachment.loaded == undefined || false}
			on:load={() => (attachment.loaded = true)}
		/>
	</button>
{/if}
...
{#if showFullView}
	...
	<div
		...
		class="attachment-full-view"
		bind:this={fullViewElem}
		on:click={onFullViewClickout}
	>
		<img
