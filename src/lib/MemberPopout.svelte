<script>
	...
	export let transitionX = -5
	let transition = {
		x: transitionX,
		duration: 100
	}
	export let member
	export let dontClickOut = undefined
	let popoutWrapper
	let popoutElem
	let done = false
	...
	$: {
		if (popoutElem) {
			let boundingRect = popoutWrapper.getBoundingClientRect()
			popoutWrapper.style.top = `${Math.min(boundingRect.top, window.innerHeight-boundingRect.height-10)}px`
		}
	}
	...
	onMount(() => {
		...
		let boundingRect = popoutWrapper.getBoundingClientRect()
		popoutWrapper.style.position = 'fixed'
		popoutWrapper.style.left = boundingRect.left+'px'
		document.body.appendChild(popoutWrapper)
		popoutWrapper.style.top = Math.max(boundingRect.top, 10)+'px'
		function onClick(e) {
			if (dontClickOut) {
				if (!Array.isArray(dontClickOut)) dontClickOut = [dontClickOut]
				for (let el of dontClickOut) {
					if (el.contains(e.target)) return
				}
			}
			if (popoutElem!=e.target&&!popoutElem.contains(e.target)) {
				dispatch('clickout')
			}
		}
...
<div transition:fade={{duration: 100}} class="member-popout-wrapper" bind:this={popoutWrapper}>
	{#if done}
		<div class="member-popout" transition:fly={transition} bind:this={popoutElem} style={member.style||null}>
			...
			<div class="member-popout-info">
				...
				{#if member.status}
					<br>
					<div class="member-popout-status">{member.status}</div>
				{/if}
				<hr>
				<h2>about me</h2>
				<pre class="member-popout-bio">
					<BasicMarkdown text={bio}></BasicMarkdown>
				</pre>
				{#if member.socials||member.email}
					...
					<div class="member-popout-social-list">
						{#if member.email}
							<a href="mailto:{member.email}">
								<div class="member-popout-social">
									...
									<p class="member-popout-social-label">{member.email}</p>
								</div>
							</a>
						{/if}
						...
						<a href="https://{member.socials.tumblr}.tumblr.com">
							<div class="member-popout-social">
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M309.8 480.3c-13.6 14.5-50 31.7-97.4 31.7-120.8 0-147-88.8-147-140.6v-144H17.9c-5.5 0-10-4.5-10-10v-68c0-7.2 4.5-13.6 11.3-16 62-21.8 81.5-76 84.3-117.1.8-11 6.5-16.3 16.1-16.3h70.9c5.5 0 10 4.5 10 10v115.2h83c5.5 0 10 4.4 10 9.9v81.7c0 5.5-4.5 10-10 10h-83.4V360c0 34.2 23.7 53.6 68 35.8 4.8-1.9 9-3.2 12.7-2.2 3.5.9 5.8 3.4 7.4 7.9l22 64.3c1.8 5 3.3 10.6-.4 14.5z" fill="currentColor"/></svg>
								<p class="member-popout-social-label">Tumblr</p>
							</div>
						</a>
						{/if}
						{#if member.socials.mastodon}
							<a href="{member.socials.mastodon}">
								<div class="member-popout-social">
									<svg aria-hidden="true" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 216.4144 232.00976">
										<mask id="m-mask">
									...
									<p class="member-popout-social-label">Mastodon</p>
								</div>
							</a>
						{/if}
						{#if member.socials.github}
							<a href="https://github.com/{member.socials.github}">
								<div class="member-popout-social">
									<svg aria-hidden="true" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="1.4 1.4 33.2 33.2" enable-background="new 0 0 36 36" xml:space="preserve">
									...
									<p class="member-popout-social-label">GitHub</p>
								</div>
							</a>
						{/if}
						{#if member.socials.lastfm}
							<a href="https://last.fm/user/{member.socials.lastfm}">
								<div class="member-popout-social">
									<svg class="member-social-icon" aria-label="Last.fm" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M225.8 367.1l-18.8-51s-30.5 34-76.2 34c-40.5 0-69.2-35.2-69.2-91.5 0-72.1 36.4-97.9 72.1-97.9 66.5 0 74.8 53.3 100.9 134.9 18.8 56.9 54 102.6 155.4 102.6 72.7 0 122-22.3 122-80.9 0-72.9-62.7-80.6-115-92.1-25.8-5.9-33.4-16.4-33.4-34 0-19.9 15.8-31.7 41.6-31.7 28.2 0 43.4 10.6 45.7 35.8l58.6-7c-4.7-52.8-41.1-74.5-100.9-74.5-52.8 0-104.4 19.9-104.4 83.9 0 39.9 19.4 65.1 68 76.8 44.9 10.6 79.8 13.8 79.8 45.7 0 21.7-21.1 30.5-61 30.5-59.2 0-83.9-31.1-97.9-73.9-32-96.8-43.6-163-161.3-163C45.7 113.8 0 168.3 0 261c0 89.1 45.7 137.2 127.9 137.2 66.2 0 97.9-31.1 97.9-31.1z"/></svg>
									<p class="member-popout-social-label">Last.fm</p>
								</div>
							</a>
						{/if}
					</div>
