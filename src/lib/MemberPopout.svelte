<script>
	...
	$: {
		if (popoutElem) {
			...
			popoutWrapper.style.top = `${Math.min(
				boundingRect.top,
				window.innerHeight - boundingRect.height - 10
			)}px`
		}
	}
...
<div
	...
	class="member-popout-wrapper"
	bind:this={popoutWrapper}
>
	{#if done}
		<div
			class="member-popout"
			transition:fly={transition}
			bind:this={popoutElem}
			style={member.style || null}
		>
			<div class="member-popout-banner" style:background-color={member.color} />
			<Avatar class="member-popout-avatar" {member} />
